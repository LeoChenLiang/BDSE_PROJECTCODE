...
datapath = 'BDSE_26_t3_project/data/predicted_data/ptt'
...
# 迴圈計算比例   
for file in files:
    df = pd.read_csv(f'BDSE_26_t3_project/data/predicted_data/ptt/{file}')
...
header_df.to_csv("ptt_data(output)/final.csv")
...
df_TAI_new.to_csv("ptt_data(output)/final_merge.csv")



# 讀入merge檔
data = pd.read_csv("ptt_data(output)/final_merge.csv")
X = pd.DataFrame(data)
...
# 去除空值
A_drop = A.dropna()
A_drop.to_csv("ptt_data(output)/final_sumweek.csv")
